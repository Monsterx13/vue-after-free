  ...
  register: function (input, name, _args, ret) {
    ...
    if (input instanceof BigInt) {
      ...
      if (!syscalls.map.has(input)) {
        // Log a warning but do NOT throw — a missing syscall wrapper should
        // not crash the entire script. Callers guard with try/catch or null checks.
        if (typeof debug === 'function') {
          debug("Syscall id ".concat(input, " not found !! (").concat(name, ")"));
        }
        return null;
      }
      id = new BigInt(input);
...
var syscalls = {
  ...
  init: function (addr) {
    var offset = 0;
    var count = 0x80000; // increased from 0x40000 to cover larger libkernel on all FW
    var view = mem.view(addr);
    var start_offset = 0;
    var pattern_idx = 0;
    while (offset < count) {
