...
function createWorker(rop, loop_size, pipe_0, pipe_1, ready, done, signal_buf) {
  return {
    rop: rop,
    loop_size: loop_size,
    pipe_0: pipe_0,
...
function create_workers () {
  ...
  for (var i = 0; i < IOV_THREAD_NUM; i++) {
    ...
    var worker = createWorker(ret.rop, ret.loop_size, pipe_0, pipe_1, ready, done, signal_buf)
  ...
  for (var i = 0; i < UIO_THREAD_NUM; i++) {
    ...
    var ret = uio_readv_worker_rop(ready, new BigInt(pipe_0), done, signal_buf)
  ...
  for (var i = 0; i < UIO_THREAD_NUM; i++) {
    ...
    var ret = uio_writev_worker_rop(ready, new BigInt(pipe_0), done, signal_buf)
  ...
  spray_ipv6_worker = createWorker(ret.rop, ret.loop_size, pipe_0, pipe_1, ready, done, signal_buf)
}
