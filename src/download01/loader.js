...
var themeFolder =
  typeof CONFIG !== "undefined" && typeof CONFIG.theme === "string"
    ? CONFIG.theme
    : "default";
...
function get_fwversion() {
  ...
  if (sysctlbyname("kern.sdk_version", buf, size, 0, 0)) {
    ...
    var version =
      byte2.toString(16) + "." + byte1.toString(16).padStart(2, "0");
    return version;
  }
  return null;
}
...
if (!is_jailbroken) {
  ...
  if (jb_behavior === 1) {
    ...
    if (
      compare_version(FW_VERSION, "7.00") >= 0 &&
      compare_version(FW_VERSION, "12.02") <= 0
    ) {
      use_lapse = true;
      lapse();
    } else if (
      compare_version(FW_VERSION, "12.50") >= 0 &&
